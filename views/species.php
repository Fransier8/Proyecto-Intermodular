<main class="flex-fill container-fluid bg-orange-300 d-flex flex-column">
    <section class="row flex-fill">
        <?php
        require 'views/aside.php';
        ?>
        <section class="col p-3">
            <div class="d-flex align-items-center justify-content-between mb-2">
                <h1 class="mb-0">Especies</h1>
                <a href="<?= BASE_URL ?>crear_especie" class="btn bg-orange-primary rounded-pill btn-lg px-4">
                    Crear especie
                </a>
            </div>
            <h4>Búsqueda y filtros</h4>
            <form class="row g-2 align-items-end">
                <div class="col-12 col-md-3">
                    <label class="form-label">Ordenar por</label>
                    <select class="form-select" name="order">
                        <option value="name_asc">Nombre A–Z</option>
                        <option value="name_desc">Nombre Z–A</option>
                    </select>
                </div>
                <div class="col-12 col-md-7">
                    <label class="form-label">Buscar</label>
                    <input name="search" type="text" class="form-control" placeholder="Buscar por nombre">
                </div>
                <div class="col-12 col-md-2">
                    <button type="submit" class="btn bg-orange-primary w-100">
                        Buscar
                    </button>
                </div>
            </form>
            <section id="species-container" class="container-fluid mt-4">
                <?php
                require 'views/lists/species_list.php';
                ?>
            </section>
        </section>
    </section>
</main>
<script>
    document.addEventListener("DOMContentLoaded", function () {
        const form = document.querySelector("form");
        const speciesContainer = document.getElementById("species-container");

        function fetchSpecies(page = 1) {
            let formData = new FormData(form);
            let params = new URLSearchParams(formData);
            params.append("ajax", "1");
            params.append("page", page);

            fetch("<?= BASE_URL ?>especies?" + params.toString())
                .then(res => res.text())
                .then(html => {
                    speciesContainer.innerHTML = html;
                });
        }

        speciesContainer.addEventListener('click', function (e) {

            if (e.target.closest('.page-link')) {
                e.preventDefault();
                const page = e.target.closest('.page-link').dataset.page;
                fetchSpecies(page);
            }

            if (e.target.closest('.delete-btn')) {
                const btn = e.target.closest('.delete-btn');
                const id = btn.dataset.id;

                if (!confirm('¿Seguro que quieres eliminar esta especie?')) {
                    return;
                }

                fetch("<?= BASE_URL ?>eliminar_especie?ajax=1", {
                    method: "POST",
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded'
                    },
                    body: "id=" + id
                })
                    .then(res => res.json())
                    .then(data => {

                        if (!data.success) {
                            alert("Error: " + data.message);
                            return;
                        }

                        fetchSpecies();
                    });
            }
        });

        form.addEventListener("submit", function (e) {
            e.preventDefault();
            fetchSpecies(1);
        });

        fetchSpecies();
    });
</script>