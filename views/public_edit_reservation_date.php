            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a
                            href="<?= BASE_URL ?>modificar_solicitud_reserva/<?= $reservation['id'] ?>">Modificar
                            reserva</a></li>
                    <li class="breadcrumb-item active" aria-current="page">Modificar fecha solicitud reserva</li>
                </ol>
            </nav>
            <h1>Modificar fecha de la solicitud de reserva</h1>
            <article class="row g-4">
                <div class="col-12 col-md-12 fs-5">
                    <form action="<?= BASE_URL ?>modificar_fecha_solicitud_reserva" method="post" class="row g-4">
                        <div class="col-12 col-md-12 fs-5">
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        form.addEventListener("submit", function (e) {
            ...
            const sameDayReservations = existingReservations.filter(r =>
                r.date == date && parseInt(r.user_id) == parseInt(userId) &&
                r.status != "cancelada" &&
                r.status != "denegada" &&
                parseInt(r.id) !== parseInt(reservationId)
            );
            ...
            const animalReservationsWeek = existingReservations.filter(r => {
                if (r.status == "cancelada" || r.status == "denegada") return false;

                const reservationDate = new Date(r.date);

                return (
                    parseInt(r.id) !== parseInt(reservationId) && parseInt(r.user_id) == parseInt(userId) &&
            ...
            const roomReservationsWeek = existingReservations.filter(r => {
                if (r.status == "cancelada" || r.status == "denegada") return false;
